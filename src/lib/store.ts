...
export async function getCollege(slug: string) {
  if (hasDatabase) {
    return prisma.college.findUnique({ where: { slug }, include: { courses: true, reviews: { orderBy: { createdAt: "desc" } } } });
  }
...
export async function getCollegesByIds(ids: string[]) {
  if (hasDatabase) {
    return prisma.college.findMany({ where: { id: { in: ids } }, include: { courses: true } });
  }
...
export async function predictColleges(exam: string, rank: number) {
  const pool = hasDatabase
    ? await prisma.college.findMany({ where: { exams: { has: exam }, rankCutoff: { gte: rank } }, orderBy: [{ rankCutoff: "asc" }, { rating: "desc" }] })
    : colleges.filter((college) => college.exams.includes(exam) && rank <= college.rankCutoff).sort((a, b) => a.rankCutoff - b.rankCutoff);
...
export async function listQuestions() {
  if (hasDatabase) {
    return prisma.question.findMany({ include: { answers: true, college: { select: { name: true } } }, orderBy: { createdAt: "desc" } });
  }
